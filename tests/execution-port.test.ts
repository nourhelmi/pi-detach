...
import { prepareLaunch } from '../src/tools/bg-agent.ts';
...
test('recursive delegation uses trusted per-launch child scope, never ambient parent grant', async () => {
  ...
  try {
    ...
    assert.match(specialist.prompt, /Optionally write a handoff report/);
    assert.doesNotMatch(specialist.prompt, /Create the parent directory and write|Return this path in the final response/);
    const legacy = await prepareLaunch({ role: 'specialist', prompt: 'legacy', harness: 'native', model: 'openai-codex/example', resultPath: '/worker/legacy-result.md' }, 'legacy');
    assert.match(legacy.prompt, /Create the parent directory and write/);
    assert.match(legacy.prompt, /Return this path in the final response/);
