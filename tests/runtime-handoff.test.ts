...
for (const status of ['PASS', 'FAIL', 'BLOCKED', 'unknown', 'missing']) test(`public managed handoffs ${status}: launch, list, delivery and reload expose captured report`, async t => {
  ...
  const result = status === 'missing' ? null : { path, sha256: 'a'.repeat(64), attempt: 1, status, integrity: 'intact', proof: 'unknown', tested: null, limitation: 'Captured worker report, not independent verification.' };
  ...
  const expect = (text: string) => { assert.match(text, /Current status:/); if (result) { assert.ok(text.includes(path)); assert.ok(text.includes(`Worker status: ${status}`)); assert.doesNotMatch(text, /Claims:|Evidence:|Remaining risk:/); assert.match(text, /proof: unknown/); assert.match(text, /Tested: unknown/); } else assert.match(text, /No current captured result/); };
...
test('formatter reports actual tested metadata and does not hide invalid report integrity', () => {
  const text = formatHandoff({ status: 'done', result: { path: '/captured/result.md', sha256: 'hash', attempt: 2, status: 'PASS', integrity: 'invalid', proof: 'unknown', tested: { surface: { revision: 'actual-revision' }, producer: 'host', limitations: ['git-visible only'] }, limitation: 'Not an independent verdict' } });
